...
bool lireFichier(int & Dr, int & Nr, double & mr, 
				int & lr, double & Lr,
				double & kr, double & ar, int & nr, double & Fr, double & qr, double & fp2r,
				int & Rr, int & tr, int & repsv);
void recursion(int Dv, int Nv, double mv, 
				int lv, double Lv,
				double kv, double av, int nv, double Fv, double qv, double fp2v,
...
void brownian_bridge(double * Brown, int lv, int nv, double fp2v);
