...
double getfitness(double p2, double k, double a)
{
	...
	{
		theta = (-b + sqrt(b*b - 4*c)) / 2;
		if (theta < 0)
			theta = (-b - sqrt(b*b - 4*c)) / 2;
	 }
	cout << "theta: " << theta << "\n";
	return( 1 - a + a * theta * pow((p2 - 1/sqrt(theta)),2));
}
...

{
	...
	// create fitness lookup tabl
	double p2prop = 0.0;
	for (i = 0; i < (lv + 1); i++)
	{
		Wtable[i] = getfitness(p2prop, kv, av);
		cout << i << "\tp2: " << p2prop  << "\tfitness: " << Wtable[i] << "\n";
		p2prop += 1.0 / lv;
	}
